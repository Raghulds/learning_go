...
func fileSig1(path string) (string, error) {
...
func parseSigFile1(r io.Reader) (map[string]string, error) {
...
func TaxiCheckConcurrent() {
	...
	for name, signature := range sigs {
		go func() {
			sigworker(rootDir, name, signature, ch)
		}()
		// close(ch)
	}
...
func sigworker(rootDir, name, signature string, ch chan Result) {
	r := Result{fileName: name}
	defer func() {
		ch <- r
	}()

	fileName := path.Join(rootDir, name) + ".bz2"
	sig, err := fileSig1(fileName)
	if err != nil {
		r.err = err
	} else {
